...
def train_epoch(summary, summary_dev, cfg, args, model, dataloader,
   ...
   # total_ones_per_task = [0] * num_tasks
    for step in range(steps):
        ...
        # # Initialize a set to track seen patients
        # seen_patients = set()
        # unique_indices = []

        # # Find duplicates
        # patient_counts = Counter(patient)
        # duplicates = [p for p, count in patient_counts.items() if count > 1]
        
        # #logging.info(f"Step {step}: Patient - {patient}")
        
        # # if duplicates:
        # #     logging.info(f"Duplicate Patients in Step {step}: {duplicates}")
        
        # # Loop to collect unique patients
        ...
        #     index = torch.tensor(index) if isinstance(index[0], int) else np.array(index)

        
        # # Select only unique data
        # index = index[unique_indices]
        # patient = [patient[i] for i in unique_indices]
        # image1 = image1[unique_indices]
        # image2 = image2[unique_indices]
        # target = target[unique_indices]
        # causal_attr = causal_attr[unique_indices]
        # bias = bias[unique_indices]

        # # Check for duplicates after filtering
        # patient_counts_after_filtering = Counter(patient)
        # duplicates_after_filtering = [p for p, count in patient_counts_after_filtering.items() if count > 1]

        # # if duplicates_after_filtering:
        # #     logging.warning(f"Duplicates still found in batch {step} after filtering: {duplicates_after_filtering}")
        # # else:
        # #     logging.info(f"No duplicates found in batch {step} after filtering.")

        # # Logging to confirm the retained patients
        # # logging.info(f"Step {step}: Patients after filtering: {patient}")
       ...
       # beta = 0.5
        ...
        if args.model == 'only_conf':
            total_loss = mainloss 
        elif args.model == 'nonconcatcausal_conf':
...
def run(args):
    ...
    model = ConfClassifier(cfg, pretrained = True).to(device)
    #if args.verbose is True:
    ...
    #         filtered_state_dict1 = {k: v for k, v in ckpt1.items() if k in model.state_dict() and v.size() == model.state_dict()[k].size()}
    #         model.load_state_dict(filtered_state_dict1, strict=False)
            
    #     if pre_train_path2 is not None and os.path.exists(pre_train_path2):
    #         ckpt2 = torch.load(pre_train_path2, map_location=device)
    #         filtered_state_dict2 = {k: v for k, v in ckpt2.items() if k in model.state_dict() and v.size() == model.state_dict()[k].size()}
    ...
    #         filtered_state_dict = {k: v for k, v in ckpt.items() if k in model.module.state_dict() and v.size() == model.module.state_dict()[k].size()}
