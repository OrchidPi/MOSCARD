...
device = torch.device("cuda:2" if torch.cuda.is_available() else "cpu")  # Primary device
...
def train_epoch(summary, summary_dev, cfg, args, model, dataloader,
    ...
    for step in range(steps):
        ...
        for j in range(num_conf):
            ...
            conf_loss += loss_j / accumulation_steps
        ...
        # Perform backward pass for confounder loss
        total_confounderloss = conf_loss + uniform_loss
        total_confounderloss.backward(retain_graph=True)  # Retain the graph for the second backward pass
        ...
        for t in range(num_tasks):
            ...
            loss += loss_t / accumulation_steps
        ...
        if summary['step'] % cfg.log_every == 0:
            ...
            summary_writer.add_scalar('train/conf_loss', conf_loss.item(), summary['step'])
        ...
        if summary['step'] % cfg.test_every == 0:
            ...
            summary_writer.add_scalar('dev/conf_loss', conf_loss.item(), summary['step'])
...
def test_epoch(summary, cfg, args, model, dataloader):
    ...
    for step in range(steps):
        ...
        output, conf_output = model(image)
        for t in range(len(cfg.num_classes)):
            task_targets = target[:, t]
            #print(f"target1:{task_targets}")
            total_zeros_per_task[t] += torch.sum(task_targets == 0).item()
            total_ones_per_task[t] += torch.sum(task_targets == 1).item()
