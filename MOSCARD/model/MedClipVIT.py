...
class MedCLIPVisionModelViT(nn.Module):
    ...
    def forward(self, pixel_values, return_layer=None, project=True):
        '''args:
        pixel_values: tensor with shape [bs, 3, img_size, img_size]
        '''
        # if pixel_values.shape[1] == 1: pixel_values = pixel_values.repeat((1,3,1,1))
        ...
        # # return img_embeds
        # hidden_states = output.last_hidden_state  # Shape: (N, sequence, embedding)

        # if project:
        #     hidden_states = self.projection_head(hidden_states)  # Apply projection head per token
        ...
        # Retrieve the early layer output
        early_layer_output = None
        if return_layer is not None:
            early_layer_output = hidden_states[return_layer]
            if project:
        ...
        return final_output, early_layer_output
